...
def KalmanFilterSmoother(x: tc.Tensor, mu0: tc.Tensor, L0: tc.Tensor, 
                         A: tc.Tensor, B: tc.Tensor, Gamma: tc.Tensor, Sigma: tc.Tensor, 
                         u: Optional[tc.Tensor]=None, C: Optional[tc.Tensor]=None,
    ...
    if use_pinv_instead_of_leastsq:
        K[:,:,0] = (L[:,:,0] @ B.T ) @ tc.linalg.pinv( B @ L[:,:,0] @ B.T + Gamma)
    else:
        K[:,:,0] = tc.linalg.lstsq((B @ L[:,:,0] @ B.T).T + Gamma.T, B @ L[:,:,0].T).solution.T
    ...
    V[:,:,0] = (tc.eye(p) - K[:,:,0] @ B) @ L[:,:,0]
    
    for t in range(1,n):        
        L[:,:,t] = A @ V[:,:,t-1] @ A.T + Sigma
        if use_pinv_instead_of_leastsq:
            K[:,:,t] = L[:,:,t] @ B.T @ tc.linalg.pinv(B @ L[:,:,t] @ B.T + Gamma)          #Kalman gain
        else:
            K[:,:,t] = tc.linalg.lstsq((B @ L[:,:,t] @ B.T).T + Gamma.T, B @ L[:,:,t].T).solution.T
        # The above is replaced by lstsq for numerical stability, see https://pytorch.org/docs/stable/generated/torch.linalg.pinv.html
        if u is not None:
        ...
        V[:,:,t] = (tc.eye(p) - K[:,:,t] @ B) @ L[:,:,t]                               #filtered covariance 
...
def ParamEstimLDS(x: tc.Tensor, u: Optional[tc.Tensor], Ez: tc.Tensor, Vhat: tc.Tensor, Vhat1: tc.Tensor, B: Optional[tc.Tensor]=None,
    ...
    if B is None:
        if use_pinv_instead_of_leastsq:
            B = tc.nansum(Ezx,dim = 2).T @ tc.linalg.pinv(tc.sum(Ezz, dim = 2))    
        else:
            B = tc.linalg.lstsq(tc.sum(Ezz, dim=2).T, tc.nansum(Ezx, dim=2)).solution.T
        if B is None:
            raise EM_Error('For some reason, B is still None, even though it was explicitly calculated.')
        # The above is replaced by lstsq for numerical stability, see https://pytorch.org/docs/stable/generated/torch.linalg.pinv.html
    
    G = tc.nanmean(x2, dim=2) - B @ tc.nanmean(Ezx, dim=2) - tc.nanmean(Ezx, dim=2).T @ B.T + B @ tc.mean(Ezz, dim=2) @ B.T
    G = tc.diag(tc.diag(G))
    ...
    # The above is replaced by lstsq for numerical stability, see https://pytorch.org/docs/stable/generated/torch.linalg.pinv.html
    ASA = A_extended.T @ SA
    # GB =  tc.linalg.inv(G) @ B
    GB = tc.linalg.solve(G, B)
    ...
    ELL = -(T *tc.log(tc.linalg.det(S)) + T * tc.log(tc.linalg.det(G)) - 2 * tc.trace(SA.T @ Ezz1_extended[:,:,0]) 
            + tc.trace(ASA @ Ezz_extended[:,:,0]) + tc.trace( BGB @ Ezz[:,:,0]) + tc.trace(tc.linalg.solve(G, x2[:,:,0]))
            - 2 * tc.trace(GB @ Ezx[:,:,0])) /2
    
    for t in range(1,T-1):
        if not tc.isnan(x2[:,:,t]).any():
            ELL = ELL - (tc.trace(tc.linalg.solve(S, Ezz[:,:,t])) - 2 * tc.trace(SA.T @ Ezz1_extended[:,:,t])
            + tc.trace( ASA @ Ezz_extended[:,:,t]) + tc.trace( BGB @ Ezz[:,:,t]) + tc.trace(tc.linalg.solve(G, x2[:,:,t]))
            - 2 * tc.trace(GB @ Ezx[:,:,t] )) / 2
    
    if not tc.isnan(x2[:,:,T-1]).any():
        ELL = ELL - (tc.trace(tc.linalg.solve(S, Ezz [:,:,T-1]))
            + tc.trace(BGB @ Ezz[:,:,T-1]) + tc.trace(tc.linalg.solve(G, x2[:,:,T-1]))
            - 2 * tc.trace(GB @ Ezx[:,:,T-1])) /2
    
    # The above is replaced by solve for numerical stability, see https://pytorch.org/docs/stable/generated/torch.linalg.inv.html#torch.linalg.inv

    C = A_extended[:,p:] if u is not None else None
    A = A_extended[:,:p]

    return mu0, A, B, C, S, G, ELL


def stable_EM_algorithm(x: tc.Tensor, u: Optional[tc.Tensor], 
    ...
    for init in range(n_initializations):
        ...
        B = tc.randn(observation_size, latent_size)    
        ...
        if u is None:
            ...
            C = tc.rand(latent_size, u.shape[0])

        #try E-step and M-step once to check if the initialization works
        Ez, Vhat, Vhat1 = KalmanFilterSmoother(x, mu0, Sigma, A, B, Gamma, Sigma, u, C, use_pinv_instead_of_leastsq=use_pinv_instead_of_leastsq)
        try:
        ...
        for i in iterator:
            ...
            try:
                ...
                if isinstance(iterator, tqdm):
                    iterator.set_description(' - '.join((pbar_descr, f'Initialization {init} EM Error: Ezz and/or Ezz1 contained inf values.')))
                    iterator.close()
                break
            except _LinAlgError:
                if isinstance(iterator, tqdm):
                    iterator.set_description(' - '.join((pbar_descr, f'Initialization {init} EM Error: Gamma not invertible.')))
                    iterator.close()
                break
            ...
            if i>0:
                LLR=ELL[i]-ELL[i-1]
                if isinstance(iterator, tqdm):
                    iterator.set_description(' - '.join((pbar_descr, f'Initialization {init} LLR={LLR:.4f}')))
            if not (ELL[i].isnan() or ELL[i].isinf()):
                ...
                if isinstance(iterator, tqdm):
                    iterator.set_description(f'{pbar_descr} - Initialization {init} EM Error: expected log likelihood is nan or inf.')
                    iterator.close()
                break
...
if __name__=='__main__':
    ...
    inputs = None
    A, B, C, Gamma, Sigma, mu0, ELL = stable_EM_algorithm(data.T, inputs, 3)
    plt.plot(ELL)
    plt.savefig('ELL.png')
