...
def train_project(project_name: str, n_runs: int=1, include_participants: Optional[list]=None,
    ...
    if os.path.exists(experiment_path):
        answer = input(f'Model path {experiment_path} already exists. Delete/Overwrite/Update/Abort? (d/o/u/a)')
        if answer=='d':
    ...
    print(f'Running {len(configs)} jobs, {n_workers} in parallel. Proceeds in {wait} seconds.')
    time.sleep(wait)
