...
from ema_forecast_control.simple_models.simple_models import get_class

tc.set_num_threads(1)

def get_parser():
    parser = argparse.ArgumentParser(description="ema_forecast_control Simple Model Training")
    ...
    # model
    parser.add_argument('--model', '-ml', type=str, default='VAR1')
...
def train_simple_model(args: dict):
    ...
    model_class = get_class(args['model'])
    ...
    if model.optimized:
        ...
        loss_df = pd.DataFrame(data=[[model.loss.item(), val_loss.item()]], index=[0], columns=['epoch_loss', 'validation_loss'])
        loss_df.to_csv(os.path.join(save_path, 'loss.csv'))
        log.info(f'{type(model).__name__} training was successful, model saved.')
    else:
        log.error(f'{type(model).__name__} training was not successful, model not saved.')
    return save_path



if __name__ == '__main__':
    train_simple_model(get_args())