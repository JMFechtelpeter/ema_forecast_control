...
class AutoregressiveTransformer(nn.Module):
    ...
    def __init__(self, args: Optional[dict]=None,
        ...
        if args is not None:
            # self.args = training_utils.complement_args(args)
    ...
    def forward_transformer(self, src: tc.Tensor, tgt: tc.Tensor, src_mask: Optional[tc.Tensor]=None, tgt_mask: Optional[tc.Tensor]=None) -> tc.Tensor:
    ...
    def get_decoder_mask(self, dim1: int, dim2: int, device=None) -> tc.Tensor:
        if device is None:
            device = self.device    
