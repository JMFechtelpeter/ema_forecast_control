...
import pandas as pd
from time_series_transformer import AutoregressiveTransformer
try:
    sys.path.append('../..')
...
class Saver:
    def __init__(self, writer: SummaryWriter, save_path: str, args: Namespace, 
                 train_dataset: MultimodalDataset, test_dataset: Optional[MultimodalDataset]):
        self.writer = writer
    ...
    def update(self, model: AutoregressiveTransformer, epoch: int, epoch_loss: float, validation_loss: float):
    ...
    def save_info(self, model: AutoregressiveTransformer, epoch: int, epoch_loss: float, validation_loss: float):
        self.update(model, epoch, epoch_loss, validation_loss)
        with tc.no_grad():           
    ...
    def save_loss_terms(self):
        ...
        if self.current_model is not None:
            ...
            loss_df['total_grad_norm'] = total_norm.item()
        else:
            total_norm = None
            loss_df['total_grad_norm'] = None
    ...
    def prediction_plot(self):       
        if self.current_model is not None:
            ...
            self.current_model.plot_prediction(obs, inputs, ylim=(0.5,7.5))
            return plt.gcf()
        else:
            raise ValueError('No model to generate trajectories')
    ...
    def ahead_prediction_plot(self):
        if self.current_model is not None:
            ...
            self.current_model.plot_generated_against_obs(obs, inputs,
                                                            ...
                                                            prewarm_kwargs={'alpha':1}, ylim=(0.5,7.5))
        else:
            raise ValueError('No model to generate trajectories')
        return plt.gcf()
